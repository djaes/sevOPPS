...
/**
 * Client for the Baian Payment Gateway
 *
 * PHP dependencies:
 * PHP >= 5.2.0
 * PHP CURL library
 ...
 * @copyright Baian 2012-2016
 * @package GatewayClient
 *
 */

/**
 * Client class for the Baian Gateway
 *
 * This class allows a client to manage several transactions with the Baian
 * Gateway.
 */
class GatewayClient {
  private $url;
 ...
 * We only use this class for its name, to separate between PHP Exceptions and
 * our own exception.
 */
...
/**
 * Transaction class for the Baian Gateway
 *
 * This class allows a client to easily build any request for the Baian
 * Gateway.
 */
class GatewayTransaction {
   ...
   *          unique reference to current transaction request.
   * @param string|null $customerIP
   *          IPv4 in Dot-decimal notation. Default value is remote IP or
   *          '127.0.0.1'.
   */
   ...
   */
  public function setAccountInformation($accountRoutingNumber, $accountNumber, $accountHolderName, $accountBankAccountType,
      $accountACHCheckType) {
   ...
   * mandatory, all sales
   *
   * @param string|null $shopperName
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperAddress
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperZipcode
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperCity
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperState
   *          For US, Canada, Australia, etc... Use null or 'NA' when
   *          unavailable (it seems no states are called 'NA').
   * @param string|null $shopperCountryCode
   *          2 letters country code as described in ISO 3166-1. Use null or
   *          'ZZ' when not unavailable ('NA' stands for Namibia).
   * @param string|null $shopperPhone
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperEmail
   *          Use null or 'NA' when unavailable.
   * @param string|null $shopperBirthDate
   *          Shopper birth Date in format YYYYMMDD. Use null when unavailable.
   * @param string|null $shopperIDNumber
   *          Customers document (passport number, ID number,taxpayer ID,... ).
   *          Use null when unavailable.
   *
   */
  public function setShopperInformation($shopperName = null, $shopperAddress = null, $shopperZipcode = null, $shopperCity = null, $shopperState = null,
      $shopperCountryCode = null, $shopperPhone = null, $shopperEmail = null, $shopperBirthDate = null, $shopperIDNumber = null) {
    ...
    !isset($shopperBirthDate) ? null : $this->requestObject->shopperBirthDate = $shopperBirthDate;
    !isset($shopperIDNumber) ? null : $this->requestObject->shopperIDNumber = $shopperIDNumber;
  }
   ...
   */
  public function setOrder($orderAmount = null, $productID = null, $comment = null, $orderDescription = null) {
   ...
   *
   */
  public function setShippingAddress($shipToName = null, $shipToAddress = null, $shipToZipcode = null, $shipToCity = null, $shipToState = null,
      $shipToCountryCode = null, $shipToPhone = null) {
   ...
   */
  public function set3DSecure($ECI = null, $XID = null, $CAVV = null, $CAVVAlgorithm = null) {
    ...
    unset($this->requestObject->PaRes);
  }
   ...
   * @param string $PaRes
   */
  public function set3DSecurePaRes($PaRes = null) {
    !isset($PaRes) ? null : $this->requestObject->PaRes = $PaRes;
    unset($this->requestObject->ECI);
    unset($this->requestObject->XID);
    unset($this->requestObject->CAVV);
    unset($this->requestObject->CAVVAlgorithm);
  }
   ...
   * MUST NOT be called if setAutomaticPartPayment or setManualSubscription have
   * already been called.
   *
   ...
   * MUST NOT be called if setAutomaticSubscription or setManualSubscription
   * have already been called.
   *
   * @param string $times
   *          (payment in 2 times)
   * @param string $interval
   *          (duration, ie "P1M" for one month)
   */
   ...
   *
   * optional, CCSale only.
   * MUST NOT be called if setAutomaticPartPayment or setAutomaticSubscription
   * have already been called.
   *
   * @param string $subscriptionType
   *          (values can be 'normal' (default) 'lifetime', 'onetime',
   *          'infinite')
   ...
   */
  public function setManualSubscription($subscriptionType, $rebillPeriod, $rebillAmount, $rebillMaxIteration, $trialPeriod = null) {
   ...
   */
  public function setAffiliate($affiliateID = null, $campaignName = null) {
   ...
   *
   * @deprecated Use PaRes directly in CCSale or CCAuthorize
   * @param string $PaRes
   */
   ...
   *
   * @param boolean $cancelSubscription
   *          True (default) to cancel associated subscription of false to leave
   *          it active
   */
  ...
  /**
   * Remove an optional parameter from the URL path if it wasn't set by
   * setInURL()
   */
